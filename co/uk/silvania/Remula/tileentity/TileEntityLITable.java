...
public class TileEntityLITable extends TileEntityChest implements IInventory
{
    private ItemStack[] lITableChestContents = new ItemStack[10];

    /** Determines if the check for adjacent chests has taken place. */
    public boolean adjacentChestChecked = false;

    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityLITable adjacentChestZNeg;

    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityLITable adjacentChestXPos;

    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityLITable adjacentChestXNeg;

    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityLITable adjacentChestZPosition;
    ...
    {
        return this.lITableChestContents[par1];
    }
    ...
    {
        if (this.lITableChestContents[par1] != null)
        {
            ItemStack var3;

            if (this.lITableChestContents[par1].stackSize <= par2)
            {
                var3 = this.lITableChestContents[par1];
                this.lITableChestContents[par1] = null;
                this.onInventoryChanged();
                return var3;
            }
            else
            {
                var3 = this.lITableChestContents[par1].splitStack(par2);

                if (this.lITableChestContents[par1].stackSize == 0)
                {
                    this.lITableChestContents[par1] = null;
                }
    ...
    public ItemStack getStackInSlotOnClosing(int par1)
    {
        if (this.lITableChestContents[par1] != null)
        {
            ItemStack var2 = this.lITableChestContents[par1];
            this.lITableChestContents[par1] = null;
            return var2;
        }
        else
    ...
    {
        this.lITableChestContents[par1] = par2ItemStack;
    ...
    public void readFromNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        this.lITableChestContents = new ItemStack[this.getSizeInventory()];
        ...
        for (int var3 = 0; var3 < var2.tagCount(); ++var3)
        {
            ...
            if (var5 >= 0 && var5 < this.lITableChestContents.length)
            {
                this.lITableChestContents[var5] = ItemStack.loadItemStackFromNBT(var4);
            }
        }
    }
    ...
    public void writeToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        for (int var3 = 0; var3 < this.lITableChestContents.length; ++var3)
        {
            if (this.lITableChestContents[var3] != null)
            {
                NBTTagCompound var4 = new NBTTagCompound();
                var4.setByte("Slot", (byte)var3);
                this.lITableChestContents[var3].writeToNBT(var4);
                var2.appendTag(var4);
            }
        }
    ...
    private void func_90009_a(TileEntityLITable par1TileEntityChest, int par2)
    {
    ...
    public void updateEntity()
    {
        ...
        if (!this.worldObj.isRemote && this.numUsingPlayers != 0 && (this.ticksSinceSync + this.xCoord + this.yCoord + this.zCoord) % 200 == 0)
        {
            ...
            while (var3.hasNext())
            {
                ...
                if (var4.openContainer instanceof ContainerLITable)
                {
                    IInventory var5 = ((ContainerLITable)var4.openContainer).getLITableChestInventory();
