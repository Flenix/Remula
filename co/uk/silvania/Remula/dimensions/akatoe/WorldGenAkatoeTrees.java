...
public class WorldGenAkatoeTrees extends WorldGenerator implements IWorldGenerator {
	private final int minTreeHeight;
	private final boolean vinesGrow;
	private final int metaWood;
	private final int metaLeaves;
	
	public WorldGenAkatoeTrees(boolean par1) {
		this(par1, 8, 0, 0, false);
	}

	public WorldGenAkatoeTrees(boolean par1, int par2, int par3, int par4, boolean par5) {
		super(par1);
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		for(int i = 0; i < 10; i++) {
			...
			(new WorldGenAkatoeTrees(false, 9, 0, 0, false)).generate(world, random, xCoord, yCoord, zCoord);
		}
	}
}