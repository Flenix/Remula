...
public class AkatoeChunkManager extends WorldChunkManager {
	private GenLayer myGenBiomes;
	private GenLayer myBiomeIndexLayer;
	private BiomeCache myBiomeCache;
	private List<BiomeGenBase> myBiomesToSpawnIn;

	protected AkatoeChunkManager() {
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList<BiomeGenBase>();
		this.myBiomesToSpawnIn.add(Remula.akatoePlainsBiome);
		this.myBiomesToSpawnIn.add(Remula.akatoeDesertBiome);
		this.myBiomesToSpawnIn.add(Remula.akatoeOceanBiome);
	}
	...
	public AkatoeChunkManager(World world) {
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		if (biome == null) {
			return Remula.akatoePlainsBiome;
		}
		return biome;
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int par2, int par3, int par4, int par5) {
		...
		for (int i = 0; i < par4 * par5; ++i) {
			if (aint[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = Remula.akatoeDesertBiome;
			}
		}
		...
		if (cacheFlag && width == 16 && length == 16 && (x & 15) == 0 && (y & 15) == 0) {
			...
			for (int i = 0; i < width * length; ++i) {
				if (aint[i] >= 0) {
					...
					//Change this to a biome
					par1ArrayOfBiomeGenBase[i] = Remula.akatoeOceanBiome;
				}
			}
			return par1ArrayOfBiomeGenBase;
		}
