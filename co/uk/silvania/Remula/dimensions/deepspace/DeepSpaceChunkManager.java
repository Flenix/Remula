...
public class DeepSpaceChunkManager extends WorldChunkManager {
	private GenLayer myGenBiomes;
	private GenLayer myBiomeIndexLayer;
	private BiomeCache myBiomeCache;
	private List<BiomeGenBase> myBiomesToSpawnIn;

	protected DeepSpaceChunkManager() {
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList<BiomeGenBase>();
		this.myBiomesToSpawnIn.add(Remula.whiteAsteroidBiome);
		this.myBiomesToSpawnIn.add(Remula.brownAsteroidBiome);
	}
	
	public DeepSpaceChunkManager(long seed, WorldType worldtype) {
		this();
		GenLayer[] agenlayer = DeepSpaceGenLayer.makeTheWorld(seed);
		this.myGenBiomes = agenlayer[0];
		this.myBiomeIndexLayer = agenlayer[1];
	}
	
	public DeepSpaceChunkManager(World world) {
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		if (biome == null) {
			return Remula.whiteAsteroidBiome;
		}
		return biome;
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int par2, int par3, int par4, int par5) {
		...
		for (int i = 0; i < par4 * par5; ++i) {
			if (aint[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = Remula.whiteAsteroidBiome;
			}
		}
