...
public class WorldGenIndikuTrees1 extends WorldGenerator implements IWorldGenerator {
	private final int minTreeHeight;
	private final boolean vinesGrow;
	private final int metaWood;
	private final int metaLeaves;
	
	public WorldGenIndikuTrees1(boolean par1) {
		this(par1, 8, 0, 0, false);
	}

	public WorldGenIndikuTrees1(boolean par1, int par2, int par3, int par4, boolean par5) {
		super(par1);
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
		...
		if (par4 >= 1 && par4 + l + 1 <= 256) {
			...
			if (!flag) {
				...
				boolean isSoil = (soil != null && soil.canSustainPlant(par1World, par3, par4 - 1, par5, ForgeDirection.UP, (IndikuSaplings1)RemulaBlocks.indikuSaplings1));
				if (isSoil && par4 < 256 - l - 1) {
							...
							for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2) {
								...
								if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || par2Random.nextInt(2) != 0 && k1 != 0) {
									...
									if (block == null || block.canBeReplacedByLeaves(par1World, j2, j1, l2)) {
										this.setBlockAndMetadata(par1World, j2, j1, l2, RemulaBlocks.indikuLeaves1.blockID, this.metaLeaves);
									}
								}
							}
						}
					...
					for (j1 = 0; j1 < l; ++j1) {
						...
						if (k1 == 0 || block == null || block.isLeaves(par1World, par3, par4 + j1, par5)) {
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, RemulaBlocks.indikuLogs1.blockID, this.metaWood);
						}
					}
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		for(int i = 0; i < 10; i++) {
			...
			(new WorldGenIndikuTrees1(false, 9, 0, 0, false)).generate(world, random, xCoord, yCoord, zCoord);
		}
	}
}