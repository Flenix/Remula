...
import co.uk.silvania.Remula.dimensions.indiku.WorldGenIndikuTrees2;
...
public class IndikuSaplings2 extends BlockFlower {
	
	public static final String[] WOOD_TYPES = new String[] {"IndikuLogs2"};
	
	@SideOnly(Side.CLIENT)
	private Icon[] icons;
	
	public IndikuSaplings2(int i, int j) {
		super(i);
		float var3 = 0.4F;
	...
	public void growTree(World par1World, int par2, int par3, int par4, Random par5Random) {
		...
		if (l == 1) {
			...
			for (i1 = 0; i1 >= -1; --i1) {
				for (j1 = 0; j1 >= -1; --j1) {
					if (this.isSameSapling(par1World, par2 + i1, par3, par4 + j1, 3) && this.isSameSapling(par1World, par2 + i1 + 1, par3, par4 + j1, 3) && this.isSameSapling(par1World, par2 + i1, par3, par4 + j1 + 1, 3) && this.isSameSapling(par1World, par2 + i1 + 1, par3, par4 + j1 + 1, 3)) {
						object = new WorldGenIndikuTrees2(true, 10 + par5Random.nextInt(20), 3, 3, false);
						flag = true;
						break;
					}
				}
			...
			if (object == null) {
				j1 = 0;
				i1 = 0;
				object = new WorldGenIndikuTrees2(true, 4 + par5Random.nextInt(7), 3, 3, false);
			}
		} else {
			object = new WorldGenIndikuTrees2(true); // Changed //
